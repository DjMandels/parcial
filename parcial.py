...
para determinar que informe hacer, tenga en cuenta lo siguiente:
    ...
    1- Tome el ultimo numero de su DNI Personal (Ej 4) y realiza ese informe (Ej, Realizar informe 4) 2
...
class App(customtkinter.CTk):
    ...
    def btn_agregar_on_click(self):
        ...
        if peso.isalpha() or peso =="" or float(peso) < 0: 
            alert("Alerta","Debe Ingresar un valor numerico mayor a cero")
            
        else:
            tipo = self.combobox_tipo_de_peso.get()
            alert("Alerta","Ingreso el valo de " + str(peso) + " " + tipo + " correctamente")
            lista = self.lista_pesos


            
            if tipo=="Gramos":
                peso_guardado  = str(float(peso))
            else:
                peso_guardado  = float(peso) * self.oz
            
            ##1 gramo son 0.035274 oz
            ##1 oz son 28.3495 gramos


            lista.append(peso_guardado)
    ...
    def btn_mostrar_on_click(self):
        ...
        for i in lista:
            ...
            if tipo=="Gramos":
                print(str(posicion) + "-" + str(i))
            else:
                pasaje  = float(i) / self.oz
                print(str(posicion) + "-" + str(pasaje))
    ...
    def btn_informar_on_click(self):
        ...
        ##7- Informar la cantidad de articulos que NO superan el peso promedio
        gramos = pasaje * self.oz
        concuerdan = 0
        for i in lista:
            if float(i) < float(gramos):
                print(str(i))
                concuerdan += 1
        
        if(concuerdan > 0):
            print("cantidad de articulos que NO superan el peso promedio = " + str(concuerdan))
        else:
            print("No se ingresaron articulos que NO superan el peso promedio")
        pass
